...
`prune-sessions` does not sweep the other stores. `session-title` and `shared-seen` stay small because only bound or forked sessions ever write them (14 and 10 entries on a machine holding 2,318 pid records). `term-sessions` is the one that does grow - it is keyed by `TERM_SESSION_ID`, which iTerm2 issues per tab, so it accumulates at roughly the rate tabs are opened (634 files and 645 rows on that same machine). It is left alone for now because its key is a terminal id, not a session id, so the liveness check that makes the sweep safe does not apply to it.
