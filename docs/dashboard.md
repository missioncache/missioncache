...
- **Waiting on people** - asks grouped by age band (under 7d, 7-13d, 14-29d, 30d+), and within a band by person, most asks first. Someone holding asks in several bands appears in each one, showing only that band's asks, with a `+N elsewhere` badge - so a recent ask is never hidden behind an old one. Filters (all, no reply over 7d, 30d+, 3 or more), a text search across person, project and ask text, and a toggle for band order.
