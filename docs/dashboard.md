...
|------|------|---------------|
| `#attention` (default) | Attention | Where attention is owed, across projects. Three cards over one table: **On you** (your open action items plus the Waiting-on rows whose `who` names you, with mark-done and resolve), **Waiting on** (the asks collapsed onto the few people holding them, most first), **Stalled** (projects with asks that have had no reply for 7 days or more, in one of two states - `chase` when you worked the project within 3 days, `parked` when you did not, which need opposite actions). The table is one row per project carrying items on you, asks on others, a no-reply share meter, the 7d+ count, and when it was last worked; the project name is a link to its page and the rest of the row expands to show your own rows first, then everyone else's. A **Today** strip below the fold carries tracked time, commits, LOC and an hourly sparkline. `#today` redirects here. |
...
- **Header stats card** - total time (Claude JSONL-only, capped at wall-clock elapsed), task count, session count, commit count, LOC added/removed. The time figure is *not* MissionCache heartbeat time - it is pure JSONL time, so it reflects actual Claude Code usage whether or not a project was loaded.
- **Hourly chart** - a 24-hour bar chart of active time per hour, using `max(task_seconds, claude_seconds)` so Claude-only hours still show up even without heartbeats. Status badges on each row indicate whether the task was Active, Done, or Untracked (Claude activity with no MissionCache project loaded).
...
- **History source toggle** - the "All / Tracked Only" switch at the top of the history card toggles between showing merged MissionCache+Claude time (the default) and MissionCache-only time. Useful when you want to see "how much did I spend on tracked projects this week" without Claude-only noise.
...
- The user worked on the project from a cwd outside `~/.missioncache/active/<project>/` and without loading the project at session start (covered in `architecture.md` invariants).
- The user spent time in the repo but outside the MissionCache directory tree (for example, editing source files in `src/`).
...
| GET | `/api/today` | Cross-project attention, split by who owes the work: `on_me` (bucketed overdue / due-soon / other), `on_others` (commitments + Waiting-on rows, grouped by project, each row carrying `mine` and a normalized `who_primary`), counts, and per-project blocks with a derived at-risk flag, `last_worked_on` / `days_since_worked`, checklist progress (`completed_count` / `total_count` / `completion_pct` / `next_up`), and a ticket reference. |
...
The split is by **who owes the work**, not by which table stored the row, so a Waiting-on row whose `who` cell names the owner (`Tomer`, `Me (...)`) carries `mine: true` and counts on his side: into `open_count` and `counts.on_me`, and out of `on_others_count`, `stale_on_others_count` and `counts.stale`. It stays inside its project's `rows` list, because the resolve path addresses it by `task_id` + `row_index` either way. Group `count` and `stale_count` describe the other-people side only, so his own overdue row cannot fill a bar that reads as "someone else is slow"; `mine_count` carries his.

**"Late" spans both kinds.** `counts.overdue` and each project's `overdue_count` count everything of *his* that has crossed its line, on the same scale the endpoint uses for sorting: an action item past its due date, and equally a Waiting-on row naming him that has passed the 7-day threshold. Both counts reach across the two lists his late work lives in (action items in `on_me`, waiting rows in `on_others`) without double-counting either. This was a real defect until 2026-07-26: the counts read the action items only, so his own work could sit a week past its line with a colleague blocked on it while the view reported "0 overdue" and `at_risk` (which derives from `overdue_count`) read healthy. A colleague sitting on an ask remains THEIR latency and counts into `stale`, never into overdue, because red is reserved for "you are late".
...
Per-project `days_since_worked` (from `tasks.last_worked_on`) exists so the view can tell a project whose asks rot while it is being worked (chase them) from one whose asks rot because the project stopped (close them). Tracked minutes cannot answer that: on a measured day only 9% of tracked time attributed to a project at all, and the most attention-owed project read 0m on a day it was worked.
...
- `db.get_sessions_from_sqlite(today)` - live MissionCache sessions from SQLite (not DuckDB, for freshness).
...
- The MissionCache plugin's own slash commands (`/missioncache:new`, `/missioncache:load`, `/missioncache:save`) POST to `/api/hooks/project` to register active projects, and resolve terminal session IDs via `/api/hooks/term-session/...`.
- The MissionCache MCP server fires `/api/hooks/task-created` internally whenever a task-creation tool runs, so the dashboard syncs immediately instead of waiting up to 60s.
- `missioncache-install` wires `/api/hooks/edit-count` into `~/.claude/settings.json` as a `PostToolUse` HTTP hook with matcher `Edit|Write|NotebookEdit` whenever the dashboard component is installed. That is what populates the statusline edit counter.
...
- [`architecture.md`](./architecture.md) - if you need a refresher on the dual-DB pattern or the missioncache-db invariants this doc assumes.
- `missioncache-dashboard/missioncache_dashboard/server.py` - the source. It is long but flat; grep for any endpoint path or helper name and you will find it.
- `missioncache-dashboard/missioncache_dashboard/lib/analytics_db.py` - if you are touching aggregate queries or adding a new one.
- `missioncache-dashboard/missioncache_dashboard/lib/jsonl_parser.py` - if you are debugging JSONL time estimation, especially the gap-capping logic.
