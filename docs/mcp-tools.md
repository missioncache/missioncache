...
- `hub: str | None = None` - A bare vault note name (`aip-release-validation`). Writes or replaces the `Hub: [[name]]` header line that `/missioncache:load` follows to the project's Obsidian hub, in the canonical header position (after `**Fork of:**`, else `**Due:**`, else `**Last Updated:**`). One hub per project: a different name replaces the line, the same name is a no-op. Slashes, brackets or newlines are **rejected** with a `VALIDATION_ERROR`, because a planted header line decides which file the resume flow reads. Reported in `sections_updated` as `hub`.
