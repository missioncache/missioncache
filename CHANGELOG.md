...
- Dashboard: new REST endpoints (`/api/tasks/{id}/pm`, action-item/stakeholder/ticket/due-date CRUD, and `/api/today`). PM tables are read/written via SQLite directly (not mirrored into DuckDB). New default Today view, a full project detail page (`#project/<name>`) with Overview and Action Items tabs, and projects-list upgrades (Due column, open/overdue badge, at-risk dot).
