...
- The Attention view is three cards over one table. **On you** carries your open action items plus the Waiting-on rows whose `who` cell names you - those previously counted against "on other people" regardless of the cell, which under-reported your own plate roughly 4x while displaying your own asks under someone else's label. **Owed by** collapses the asks onto the few people holding them via a normalized `who_primary`, turning a backlog into an agenda; it removes itself when no one holds three or more rather than asserting a head the data does not have. **Rotting** joins idle asks against `days_since_worked` and flips between "worked lately, asks unanswered" (chase) and "nobody home" (close or drop), so it never renders empty. The table is one row per project - built from `projects[]`, not the on-others groups, so a project with items on you and nothing on anyone else is no longer invisible - carrying items on you, asks on others, an idle-share meter on one absolute scale, idle count, and when it was last worked; expanding a row shows your own rows first. A Today strip carries tracked time, commits, LOC, an hourly sparkline, and how much of the day is not bound to any project.
- Red is reserved for "you are late" and nothing else, so a day with nothing overdue renders no red at all; staleness reads amber and only at project level. Four `--pm-*` colour tokens were added with light-theme values that clear WCAG AA (the four `--accent-*` colours have no light override and measure 1.29:1 to 2.91:1 against `#f4f4f8`), and `.due-cell-past` / `.due-cell-soon` now use them.
...
- Each row now says where the project stands and what to pick up next, not only who owes what: "3 asks outstanding" read identically on a project at 86% and one at 0%. `projects[]` carries `completed_count` / `total_count` / `completion_pct` and `next_up`, rendered as a green progress bar plus the fraction and the next unchecked item on one clipped line. `next_up` is derived server-side and the parsed checklist (`task_modes`, several hundred items across the active projects) is deliberately not forwarded - the row needs one line of it. The row also stopped bailing on projects with no repo row: `parse_missioncache_progress` required a repo path, but that path only builds the LEGACY repo-local candidates, so a task with `repo_id` NULL reported 0 of 0 items with its files sitting in `~/.missioncache/active/<name>/`. A project's ticket reference (tickets row first, legacy `tasks.jira_key` as fallback) renders in the expanded panel next to "open project" rather than as a mostly-empty column - it is a plain label until the JIRA URL mapping is set in Settings.
- The asks meter is a comb of one tick per ask instead of a solid bar. The bar's length was a share of the largest project, which this data's range cannot express: 1 ask and 3 asks both drew as a stub a few pixels long. Ticks are countable, so three amber ticks IS "3 asks, all waiting", and the absolute scale survives because a tick is a fixed width (measured: span is exactly `12n - 3` px). Idle ticks come first so they read as one quantity. Above 20 asks it falls back to the bar, where a comb stops being countable and stops fitting.
- The Attention view is navigable and its labels say what they mean. The whole row used to be one `<button>`, which is why there was no way to open a project from the view at all: an `<a>` cannot live inside a `<button>`, so the project name could not be a link. The row is a `div` now with two controls that do different things - the caret expands (the row's only button, carrying the `aria-expanded`/`aria-controls`/label state), the name navigates - and a click anywhere else on the row is delegated to the caret, so the numbers and the meter are not dead space. Row hover shades the row (it expands); name hover underlines and colours it (it navigates). Project links now carry an optional `?tid=<task id>`, because two active projects can share a name (a fork and its parent, or a stale row left by a rename) and name lookup returned whichever sorted first - so both rows in the table opened the same page. All eight link sites go through one `projectHref` helper, the manual-refresh path re-resolves by the id already on screen rather than by name, an id that no longer resolves falls back to the name instead of reporting "not found", and links written before `tid` existed resolve exactly as they did.
- Wording. **Owed by** is now **Waiting on** (it was being read as "Owned by", which is its own evidence). **Rotting, live / Rotting, parked** is now **Stalled** plus a `chase` / `parked` state chip and a line that spells out both thresholds, instead of encoding them in an adjective. "idle" was the heading over two different columns - a share and a count - so both are named after the threshold they actually measure (`share with no reply`, `7d+`). Per-row `gates` / `from` became `blocks` / `asked by`. The Stalled card also had a live-vs-parked predicate gap: `<= 3` days against `> 7` days left a project worked 4 to 7 days ago in neither set, so it appeared nowhere on the card. Both sides now draw on the same bound. Nothing sits in that window today, so it was latent rather than visible.
- Fewer effects, two elevations, and the accessibility the view was missing. `backdrop-filter: blur(10px)` came off five cards after verifying it was invisible (they are 20% transparent over five viewport-wide radial gradients, and blurring a smooth gradient returns the same gradient - zoomed captures with and without are indistinguishable). The hover `translateY(-2px)` came off because the projects table sits inside a card, so pointing at one row lifted all twelve; rows get their own hover instead. The projects table is the only raised surface now (opaque rather than translucent, plus a shadow) so the view has a focal plane. The file had no `prefers-reduced-motion` block at all while animating the background forever - decorative animation stops under it, spinners deliberately do not (a frozen spinner reads as a hung page). Interactive elements got `:focus-visible` rings: everything on the view is a button or a link, and the custom backgrounds were painting over the UA ring, so keyboard users had nothing to follow through twelve rows.
- The row's columns are all capped with a trailing spacer. The project name used to be the flexible track, which on a wide monitor grew it to ~790px and left every project's name that far from its own first number; handing the slack to the meter instead pushed "worked" 1600px out. The whole row now stays inside ~950px however wide the window gets.
...
- The Attention view carries a real type scale. Measured before: 130 text elements, 124 of them (95.4%) between 10px and 13px, with one style (11px/400) carrying 71 - everything the same size, so nothing was emphasised. Now six sizes and eight roles, where three roles share 14px and separate by weight and figure style rather than by an invisible 1px step: a 32px display count, 22px strip metrics, a 17px card title, 14px row names / figures / body, 12px secondary text, and a 10px uppercase eyebrow. 11px leaves the Inter text entirely (native date inputs and the monospace id chip keep their own sizes on purpose), the small-text share drops to 54.6%, and the largest single style now covers 40 elements instead of 71. Row pitch went DOWN 1.4px despite the row text growing, because the header's line-height is now pinned rather than inherited as `normal`, so the pitch is a function of one rule instead of whichever child happens to be tallest. Inter Tight is loaded for the 22px and 32px sizes only, where Inter's small-text spacing reads slack; it falls back to Inter and then the system stack, so it adds no failure mode Inter did not already have. Weight 300 was requested from Google Fonts on every load and declared nowhere in the file - it is gone.
- The Today view splits by **who owes the work**, not by which record stored it. "On me" is your own open items bucketed overdue / due-soon / other. "On other people" is one list merging action items assigned to colleagues (`kind: commitment`) with Waiting-on rows (`kind: blocker`) - the two stay separate at rest (one DB-canonical with a stable id, one file-canonical maintained by the save flow) and are joined only for reading, on a shared "days past the line" scale: a commitment's line is its due date, a blocker's is the 7-day staleness threshold, and rows that have not crossed a line sort below, oldest first. Both kinds resolve from the UI by different mechanisms: a commitment has a stable id so it completes via `PUT /api/action-items/{id}`, while a Waiting-on row has none (it is hand-editable markdown) so `POST /api/tasks/{id}/waiting-on/resolve` identifies it positionally and verifies its what/who/since before removing it, answering 409 rather than resolving the wrong row when the table has shifted. Chips read ON ME / OVERDUE / ON OTHER PEOPLE / STALE, and each project's attention block carries typed counts (`open_count`, `overdue_count`, `on_others_count`, `stale_on_others_count`) instead of a warning-string list.
