...
On 2.1.232+ the harness enforces unique interactive-session names itself: when the hook's emitted title collides with a name some other live session already displays, the harness assigns a `name-word-word` variant instead. The hook cannot see the displayed title, so the recorded address then diverges from the display until the address next recomputes differently - the entry falls into the no-matching-row skip below rather than misdelivering.

`ListAgents` is the reachability authority, not the pid filter. The pid record only proves a process runs, and one `claude` process hosts many sessions, so a closed session can stay "pid-alive" indefinitely. A session absent from `ListAgents` is unreachable regardless of what the pid says, with one exception. On 2.1.234+ the tools report when your account's session list was too long to check completely, instead of letting unseen sessions look absent. When they say that, absence is evidence about the listing and not about the session, so it does not support any claim that the peer is gone.

**Sending.** For each entry:

1. Call `ListAgents` and find the row whose name is the entry's `title`.
2. `SendMessage` to the bare name first. On Claude Code 2.1.232+ a bare name matching exactly one live session delivers directly; older code rejects every bare cross-session name - and a long-lived session keeps the code it launched with, whatever `claude --version` says (it follows the install symlink, not the running process), so rejections keep happening after an upgrade until sessions restart. When the send is rejected, the error itself prints the row and a fresh ` [ref]` - re-send with exactly that (`{"to": "avc-in-house-testing [c8fc2f]"}`). Never reuse a ref from an earlier listing or error; refs are per-listing. An unchecked session list is the second thing that sends you down the ref path: `SendMessage`'s own guidance asks for the ` [ref]` when two rows share a name *or* when a session list could not be checked, so a unique name is not always enough. Message every entry: a project can legitimately have more than one live session.
3. No matching row means one of two things. Either the session is gone - closed while its shared `claude` process lives on, so the pid filter could not catch it - or, on 2.1.234+, your account's session list was too long to check completely, which `ListAgents` and `SendMessage` now say outright. Keep them apart, because they call for different words to the user. When the listing reports it could not be checked in full, say the peer could not be checked; do not call it unreachable and do not imply the notification failed. Otherwise skip that entry and tell the user in one line ("one bound session was unreachable, skipped"); do NOT block on a question. A live session whose title went stale heals itself on its next prompt, and a renamed session gets its update from the context file on its next `/missioncache:load`.
3b. TWO rows matching one title is the one case that stays a question: sending to the wrong twin is a real misdelivery, so ask which row with `AskUserQuestion`. On 2.1.232+ the harness itself keeps interactive-session names unique (a colliding start or `/rename` gets a `name-word-word` variant), so twins should only come from unmanaged background sessions that inherited a project's name without being bound or suffixed - rare, but still the ask.
